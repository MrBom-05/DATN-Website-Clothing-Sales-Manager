...
public interface HoaDonChiTietRepository extends JpaRepository<HoaDonChiTiet, UUID> {
    ...
    // user
    @Query("SELECT new com.example.websitebanquanao.infrastructures.responses.GioHangUserResponse(sp.id, spct.id, sp.ten, ms.id, ms.ten, kc.ten, hdct.soLuong, hdct.gia) FROM HoaDonChiTiet hdct JOIN hdct.idSanPhamChiTiet spct JOIN spct.idSanPham sp LEFT JOIN spct.idMauSac ms LEFT JOIN spct.idKichCo kc WHERE hdct.idHoaDon.id = :idHoaDon")
