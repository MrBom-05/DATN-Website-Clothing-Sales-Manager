...
public interface GioHangRepository extends JpaRepository<GioHang, UUID> {
    @Query("SELECT new com.example.websitebanquanao.infrastructures.responses.GioHangUserResponse(sp.id, spct.id, sp.ten, ms.id, ms.ten, kc.ten, sp.anh, ghct.soLuong, ghct.gia) FROM GioHangChiTiet ghct JOIN ghct.idGioHang gh JOIN gh.idKhachHang kh JOIN ghct.idSanPhamChiTiet spct JOIN spct.idSanPham sp LEFT JOIN spct.idMauSac ms LEFT JOIN spct.idKichCo kc WHERE kh.id = :idKhachHang")
